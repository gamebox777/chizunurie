...
paintedRouter.post("/", async (c) => {
  ...
  if (parsed.mode === "gps") {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (inserted.length > 0) {
        ...
        return { ok: true as const, points: state, gainedExp: gained };
      }
      ...
      if (bit !== 0n && curMask !== 0n && (curMask & bit) === 0n) {
        ...
        return { ok: true as const, points: state, gainedExp: expCfg.expFine };
      }
      ...
      if (existing[0]?.mode !== "gps") {
        ...
        return { ok: true as const, points: state, gainedExp: expCfg.expVisit };
      }
      ...
      const canReward =
        last === null || now - last.getTime() >= expCfg.revisitCooldownSec * 1000;
      if (canReward) {
        await tx
          .update(paintedRegions)
          ...
          .where(cellWhere);
        const revisitExp =
          bit !== 0n && curMask !== 0n ? expCfg.expFineRevisit : expCfg.expVisit;
        const state = await addExp(user.id, revisitExp, now, tx, expCfg);
        return { ok: true as const, points: state, gainedExp: revisitExp };
      }
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      return { ok: true as const, points: state, gainedExp: expCfg.expPaint };
    });
    return c.json(result);
  } catch (err) {
