...
userRouter.post("/me/country", async (c) => {
  ...
  const body = (await c.req.json().catch(() => null)) as { country?: unknown } | null;
  const raw = body?.country;
  const country =
    typeof raw === "string" && raw.length > 0 && raw.length <= 8 ? raw : null;
  if (country === null) return c.json({ error: "bad request" }, 400);
