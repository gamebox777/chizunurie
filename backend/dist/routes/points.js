...
async function guardWebRewardDisabled(userId, platform) {
    if (platform !== "web")
        return false;
...
pointsRouter.get("/reward/video", async (c) => {
    ...
    if (!user)
        return c.json({ error: "unauthorized" }, 401);
    const status = await getVideoRewardStatus(user.id, Date.now(), rewardPlatform(c.req.query("platform")));
