...
```
            https://chizunurie.unitygamebox.com  （Coolify が TLS 自動取得）
...
https://chizunurie.unitygamebox.com/api/auth/callback/google   ← 本番用（追加）
```
...
- 独自ドメインにするには、Coolify の **frontend サービスの Domains 欄**を
  `https://chizunurie.unitygamebox.com` に変更する（DNS の A レコードをVPSに向けておくこと）。
...
> 手順0 で本番用リダイレクト URI（`https://chizunurie.unitygamebox.com/api/auth/callback/google`）を
...
1. `https://chizunurie.unitygamebox.com/` が表示され、地図が出る
...
- **Google ログイン後に redirect_uri_mismatch**：手順0の本番リダイレクトURIが Google 側に
  登録されているか、公開ドメインが `https://chizunurie.unitygamebox.com/api/auth/callback/google`
