#!/usr/bin/env node
// ───────────────────────────────────────────────────────────────────────────
// build-graphics.mjs — App Store 用スクリーンショット（仮）を「必要な解像度ちょうど」で生成
//
//   node build-graphics.mjs
//
// 依存なし。HTML/Canvas を headless Chrome で正確なピクセルサイズにレンダリングする。
// play-store 版（mobile/play-store/tools/build-graphics.mjs）と同じブランド配色・地図描画を
// 使うが、App Store Connect が要求する iOS の解像度・アスペクト比に合わせて作り直す。
//
// 出力（App Store Connect にそのままアップロードできるサイズ）:
//   screenshots/iphone-6.9/{ja,en}/0N.png   1290x2796（6.9型 iPhone・★必須）×5
//   screenshots/iphone-6.5/{ja,en}/0N.png   1242x2688（6.5型 iPhone・任意）×5
//   screenshots/ipad-13/{ja,en}/0N.png      2048x2732（13型 iPad・iPad対応のため必須）×2
//
// アプリアイコン（1024x1024・アルファ無し）は実機のものを使うので生成しない：
//   graphics/icon/icon-1024.png ← ios/.../AppIcon.appiconset/AppIcon-512@2x.png をコピー済み
// ───────────────────────────────────────────────────────────────────────────
...
// projector は x/y 同一スケール（min を採用）なので、canvas のアスペクト比が変わっても
// 地図ポリゴンも塗りマスも歪まない（海の余白が増減するだけ）。iOS の縦長/正方比でも安全。
  ...
  const t =
    lang === "ja"
      ? {
          ...
          lv: "Lv.4",
          rankTitle: "全国ランキング",
          rows: [["1", "ぬりお", "12,840"], ["2", "まっぷ太郎", "9,310"], ["3", "あなた", "7,205"]],
          statTitle: "東京都 64%",
      ...
      : {
          ...
          rows: [["1", "Nurio", "12,840"], ["2", "MapTaro", "9,310"], ["3", "You", "7,205"]],
          statTitle: "Tokyo 64%",
...
// デザインは横幅 1080 を基準に組み、出力時に scale = 出力幅/1080 で等比拡大する。
// 高さ(H)はデバイスのアスペクト比に合わせて可変。垂直方向は実寸 px で計算して
// 余白なく収める（端末枠の中身＝map canvas の解像度も box にぴったり合わせる）。
function phoneHTML({ lang, headline, sub, variant, H }) {
  const W = 1080;
  const kind = lang === "ja" ? "japan" : "world"; // 日本語=日本地図 / 英語=世界地図
  const overlay = variant === "rank" ? "rank" : variant === "large" ? "stat" : "";
  const gps = variant === "gps";

  // 縦方向の実寸レイアウト（design px）
  const BH = Math.round(H * 0.2); // バンド（テキスト見出し）の高さ
  const mTop = 18, mBottom = 70, mSide = 60, pad = 16, appbarH = 96;
  const deviceH = H - BH - mTop - mBottom;
  const frameW = W - mSide * 2 - pad * 2;
  const frameH = deviceH - pad * 2;
  const mapH = frameH - appbarH;

  return `<!doctype html><html><head><meta charset="utf-8"><style>
   *{margin:0;padding:0;box-sizing:border-box}
   html,body{width:${W}px;height:${H}px;overflow:hidden;font-family:${FONT}}
   .screen{width:${W}px;height:${H}px;background:linear-gradient(160deg,#3b6fd4,#6ea0ee);
     display:flex;flex-direction:column;align-items:center}
   .band{width:100%;height:${BH}px;padding:0 78px;color:#fff;text-align:center;
   ...
   h1{font-size:66px;line-height:1.22;font-weight:800;margin:22px 0 14px;letter-spacing:.01em}
   .band p{font-size:33px;line-height:1.5;opacity:.94;font-weight:500}
   .device{margin:${mTop}px ${mSide}px ${mBottom}px;background:#1a1a1a;border-radius:54px;
     padding:${pad}px;box-shadow:0 30px 60px rgba(0,0,0,.30);width:${W - mSide * 2}px;height:${deviceH}px}
   .frame{width:100%;height:100%;background:#eef1f4;border-radius:40px;overflow:hidden;position:relative}
   .appbar{position:absolute;top:0;left:0;right:0;height:${appbarH}px;background:#fff;display:flex;
   ...
   .map{position:absolute;top:${appbarH}px;left:0;width:${frameW}px;height:${mapH}px;display:block}
   .toolbar{position:absolute;top:${appbarH + 32}px;left:30px;display:flex;gap:10px;z-index:6}
   ...
   .ptchip{position:absolute;top:${appbarH + 116}px;left:34px;color:#e03131;font-size:30px;font-weight:800;z-index:6;
   ...
   .panel{position:absolute;right:30px;top:${appbarH + 204}px;width:62%;background:rgba(255,255,255,.97);
   ...
   <div class="screen">
     ...
     <div class="device"><div class="frame">
       ${appChrome(lang, { cw: frameW, ch: mapH, overlay, gps })}
     </div></div>
   </div>
   <script>${MAP_JS}
...
function render(html, w, h, outPath, scale = 1, exactW = 0, exactH = 0) {
  ...
  // 等比拡大の丸めで ±1px ずれることがある（iPad 2048x2733 等）。App Store Connect は
  // 解像度ちょうどを要求するので sips で出力サイズを厳密に揃える（1px 程度の補正で歪みなし）。
  if (exactW && exactH) {
    execFileSync("sips", ["-z", String(exactH), String(exactW), outPath], { stdio: "ignore" });
  }
  console.log("rendered", outPath.replace(G + "/", ""), `${exactW || Math.round(w * scale)}x${exactH || Math.round(h * scale)}`);
}
...
// App Store Connect の解像度。design 幅 1080 を出力幅に等比拡大する。
//   designH = round(1080 * outH / outW) でアスペクト比を合わせ、scale = outW/1080。
const DEVICES = [
  { dir: "iphone-6.9", outW: 1290, outH: 2796, picks: [0, 1, 2, 3, 4] }, // 6.9型 iPhone（必須）
  { dir: "iphone-6.5", outW: 1242, outH: 2688, picks: [0, 1, 2, 3, 4] }, // 6.5型 iPhone（任意）
  { dir: "ipad-13", outW: 2048, outH: 2732, picks: [0, 3] }, // 13型 iPad（iPad対応のため必須）
];

for (const lang of ["ja", "en"]) {
  for (const dev of DEVICES) {
    const designH = Math.round(1080 * (dev.outH / dev.outW));
    const scale = dev.outW / 1080;
    dev.picks.forEach((idx, j) => {
      const [headline, sub, variant] = CAPTIONS[lang][idx];
      const n = String(j + 1).padStart(2, "0");
      const html = phoneHTML({ lang, headline, sub, variant, H: designH });
      render(html, 1080, designH, resolve(G, `screenshots/${dev.dir}/${lang}/${n}.png`), scale, dev.outW, dev.outH);
    });
  }
}

rmSync(TMP, { recursive: true, force: true });
console.log("\nDONE — icon は実機 (icon-1024.png) を使用。スクショは graphics/screenshots/ 以下。");
