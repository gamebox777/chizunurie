...
frontend/public/data/*.geojson（市区町村・町丁目・都道府県）
  ↓ scripts/build-mesh.mjs（約1kmメッシュ生成＋地名付与）
frontend/public/data/mesh.geojson（生成中間ファイル・約100MB・git管理外）
  ├ scripts/build-pmtiles.mjs（geojson-vt + vt-pbf + pmtiles.exe）
  │   → frontend/public/data/japan.pmtiles（単一ファイル、複数ベクターレイヤー）
  └ scripts/build-muni-stats.mjs（市区町村ごとにメッシュコードを集約）
      → frontend/public/data/muni-stats.json（塗り％表示用・約3MB）
...
- メッシュは数学的に生成（データDL不要）。`scripts/build-mesh.mjs` が市区町村ポリゴンの中で中心点が陸地に入るセルだけを採用し、全国 約37万セルを作る。
- **3次メッシュ = 緯度 1/120°・経度 1/80° の均一グリッド**。セルの安定ID（8桁の地域メッシュコード）はグリッド整数 `(ri, ci)` から数式で相互変換できる（`Map.tsx` の `meshCodeAt` / `gridFromMeshCode` / `meshCodeFromGrid`）。
- 各セルには**近似の地名**を埋め込む：中心点が入る市区町村名を基本とし、全47都道府県の町丁目（`chocho/XX_chocho.geojson`・XXは都道府県コード2桁）の `S_NAME` で上書きする。`build-mesh.mjs` は `chocho/` 内の `*_chocho.geojson` を自動で全て読む。
- メッシュコードは不変なので、旧来の `generateId` 由来IDがリロードで変わる問題は解消されている。
...
| `mesh` | 約1kmメッシュ（塗りの単位・全国約37万セル） | 9–13 |
...
町丁目・市区町村データを変更したらプロジェクトルートで実行する：

```bash
npm run build-mesh && npm run build-muni-stats && npm run build-pmtiles
```

（いずれもメモリを使うため `--max-old-space-size=8192` 付き。`build-mesh` は `chocho/` 内の
 `*_chocho.geojson` を全て読む。`build-muni-stats` は `mesh.geojson` を読んで市区町村ごとの
 塗り％用の `muni-stats.json` を生成する。テスト時は
 `node scripts/build-mesh.mjs --bbox minLng,minLat,maxLng,maxLat` で範囲限定できる）
